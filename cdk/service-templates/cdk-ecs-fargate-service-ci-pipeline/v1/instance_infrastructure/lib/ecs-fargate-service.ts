...
export class EcsFargateServiceStack extends Stack {
  constructor(scope: Construct, id: string, props: StackProps) {
    ...
    const ecrPrivatePattern =
      /^\d+\.dkr\.ecr\.[a-z0-9-]+\.amazonaws.com\/[a-z0-9-_:.]+$/i;

    if (ecrPrivatePattern.test(instanceInputs.inputs.image)) {
      //image = ecs.ContainerImage.fromEcrRepository(
      image = new ecs.EcrImage(
        Repository.fromRepositoryName(
          this,
          "EcrRepo",
          instanceInputs.inputs.image
        ),
        instanceInputs.inputs.imageTag
      );
    } else {
      image = ecs.ContainerImage.fromRegistry(
        `${instanceInputs.inputs.image}:${instanceInputs.inputs.imageTag}`
      );
    }
      ...
      services: [
        {
          ...
          discoveryName: `${instanceInputs.name}-${input.service.name}`,
        },
      ],
    };
    ...
    new CfnOutput(this, "ServiceDiscoveryName", {
      value: `${instanceInputs.name}-${input.service.name}`,
    });

    new CfnOutput(this, "ECSServiceDashboard", {
      value: `https://${
        Stack.of(this).region
      }.console.aws.amazon.com/ecs/v2/clusters/${
        environmentOutputs.outputs.ECSClusterName
      }/services/${ecsService.serviceName}/health?region=${
        Stack.of(this).region
      }`,
    });
  }
