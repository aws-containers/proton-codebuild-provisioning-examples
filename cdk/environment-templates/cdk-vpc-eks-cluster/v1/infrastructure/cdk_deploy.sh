...
# To solve this, we deploy the -eks stack stack first, then we can deploy the -cluster stack 
###
...
if [[ ${new_ns_length} -lt ${current_ns_length} ]]; then
  ...
  npm run cdk -- deploy --exclusively "${env_name}-eks" --outputs-file ./output1.json
