  ...
  source {
    buildspec = <<EOF
    {
      ...
      "phases": {
        ...
        "pre_build": {
          "commands": [
            "cd $CODEBUILD_SRC_DIR/${var.service_source_dir}",
        ...
        "build": {
          "commands": [
            ...
            "docker build -t $IMAGE_REPO_NAME:$IMAGE_TAG -f ${var.dockerfile} .",
      ...
      "artifacts": {
        "files": [
          "${var.service_source_dir}/rendered_service.yaml"
        ]
      }
    }
    EOF
    ...
    buildspec = <<EOF
    {
      ...
      "phases": {
        "build": {
          "commands": [
            "pip3 install --upgrade --user awscli",
            "aws proton --region $AWS_DEFAULT_REGION update-service-instance --deployment-type CURRENT_VERSION --name $SERVICE_INSTANCE_NAME --service-name $SERVICE_NAME --spec file://${var.service_source_dir}/rendered_service.yaml",
